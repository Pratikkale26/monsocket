...
- **Gas limits are measured per code path, not per function.** Monad bills the
  limit, not the gas used, so padding is real money and an under-estimate is a
  silent revert. Creating a room costs ~210k and updating one ~87k, so
  `setState` carries a 320k cold limit until the room is known to exist and
  120k after.
- **Reads** poll `eth_getLogs` every 250ms against `latest`, which on Monad is
  the *Proposed* block — one step ahead of finality. The public RPC caps log
  queries at 100 blocks, so catch-up happens in capped hops and re-reads state
  from storage after a gap, which is the difference between a tab you can
  background and one that silently dies.
...
| call | what it does |
|---|---|
| `MonSocket.connect({ key, contract, rpc? })` | Create a client from a burner private key. |
...
| `room.leave()` | Stop polling. There is nothing onchain to undo. |
...
Monad testnet. Verified by a 20-check two-client suite against the live chain —
presence both ways, chat, state races converging, rapid-fire nonce handling, and
a 0-MON spectator streaming everything.
